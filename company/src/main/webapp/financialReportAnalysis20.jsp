...
<body>
	...
	<div class="mid_title1">现金及等价物净增额（金额单位：元人民币）</div>
	<table style="width:97%">
	   <tbody>
	   	...
	   	<tr>
	     	...
	     	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID104"  target="_blank">a.现金及现金等价物净增加额</a></td>
	    	<c:forEach var="rsf" items="${rsfList}">
	    		<td class="mid_table_content"><div>${rsf.netIncreaseInCashAndCashEquivalentsFormated}</div></td>
	    	</c:forEach>
       	</tr>
       	<tr>
	     	<td class="mid_table_content">2</td>
	     	<td class="mid_table_content">b.现金分红金额</td>
	    	<c:forEach var="rsf" items="${rsfList}">
	    		<td class="mid_table_content"><div>${rsf.financialReportDividendsFormated}</div></td>
	    	</c:forEach>
       	</tr>
       	<tr>
		 	<td class="mid_table_content">3</td>
		 	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID105"  target="_blank">加现金分红的现金及现金等价物净增加额=a+b</a></td>
		 	<c:forEach var="rsf" items="${rsfList}">
		 		<c:set var="scopeOperation" value="0" />
		 		<c:set var="netIncreaseInCashAndCashEquivalentsAddBonus" value="${fn:substring(rsf.netIncreaseInCashAndCashEquivalentsAddBonusFormated, 0, fn:length(rsf.netIncreaseInCashAndCashEquivalentsAddBonusFormated))}" /> 
		 		<c:choose>
		 			<c:when test="${netIncreaseInCashAndCashEquivalentsAddBonus < scopeOperation}">
	    			 	<td class="mid_table_content"><div style="color: red; font-weight: bold;">${rsf.netIncreaseInCashAndCashEquivalentsAddBonusFormated}</div></td>
	    			</c:when>
	    			<c:otherwise>
	    			  	<td class="mid_table_content"><div>${rsf.netIncreaseInCashAndCashEquivalentsAddBonusFormated}</div></td>
	    			</c:otherwise>
	    		</c:choose>
	    	</c:forEach>
        </tr>
     	<tr>
		 	...
		 	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID106"  target="_blank">期末现金及现金等价物余额</a></td>
		 	<c:forEach var="rsf" items="${rsfList}">
		 		<c:set var="scopeOperation" value="0" />
		 		<c:set var="closingBalanceOfCashAndCashEquivalents" value="${fn:substring(rsf.closingBalanceOfCashAndCashEquivalentsFormated, 0, fn:length(rsf.closingBalanceOfCashAndCashEquivalentsFormated))}" /> 
		 		<c:choose>
		 			<c:when test="${closingBalanceOfCashAndCashEquivalents < scopeOperation}">
	    			 	<td class="mid_table_content"><div style="color: red; font-weight: bold;">${rsf.closingBalanceOfCashAndCashEquivalentsFormated}</div></td>
	    			</c:when>
	    			<c:otherwise>
	    			  	<td class="mid_table_content"><div>${rsf.closingBalanceOfCashAndCashEquivalentsFormated}</div></td>
	    			</c:otherwise>
	    		</c:choose>
	    	</c:forEach>
        </tr>
