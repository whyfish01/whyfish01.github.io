...
<body>
	...
	<div class="mid_title1">分红情况（金额单位：元人民币）</div>
	<table style="width:97%">
	   <tbody>
	   	...
	   	<tr>
	     	...
	     	<td class="mid_table_content">a.现金分红金额</td>
	    	<c:forEach var="rsf" items="${rsfList}">
       	...
       	<tr>
	     	...
	     	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID088"  target="_blank">b.归属于上市公司普通股股东的净利润</a></td>
	    	<c:forEach var="rsf" items="${rsfList}">
        ...
        <tr>
		 	...
		 	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID099"  target="_blank">分红率（股利支付率）=a÷b×100%</a></td>
		 	<c:forEach var="rsf" items="${rsfList}">
		 		<c:set var="scopeOperation" value="0.3" />
		 		<c:set var="dividendRate" value="${fn:substring(rsf.dividendRateFormated, 0, fn:length(rsf.dividendRateFormated)-1)}" /> <!-- 提取百分数中的数字部分 -->
		 		<c:choose>
		 			<c:when test="${dividendRate/100 < scopeOperation}">
	    			 	<td class="mid_table_content"><div style="color: red; font-weight: bold;">${rsf.dividendRateFormated}</div></td>
	    			</c:when>
	    			<c:otherwise>
	    			  	<td class="mid_table_content"><div>${rsf.dividendRateFormated}</div></td>
	    			</c:otherwise>
	    		</c:choose>
	    	</c:forEach>
        </tr>
