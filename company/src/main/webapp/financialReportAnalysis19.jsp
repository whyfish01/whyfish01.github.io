...
<body>
	...
	<div class="mid_title1">公司类型（金额单位：元人民币）</div>
	<table style="width:97%">
	   <tbody>
       	...
       	<tr>
		 	...
		 	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID086"  target="_blank">经营活动产生现金流量净额</a></td>
		 	<c:forEach var="rsf" items="${rsfList}">
		 		<c:set var="scopeOperation" value="0" />
		 		<c:set var="netCashFlowFromOperating" value="${fn:substring(rsf.netCashFlowFromOperatingFormated, 0, fn:length(rsf.netCashFlowFromOperatingFormated))}" /> <!-- 提取百分数中的数字部分 -->
		 		<c:choose>
		 			<c:when test="${netCashFlowFromOperating < scopeOperation}">
	    			 	<td class="mid_table_content"><div style="color: red; font-weight: bold;">${rsf.netCashFlowFromOperatingFormated}</div></td>
	    			</c:when>
	    			<c:otherwise>
	   	...
	   	<tr>
	     	...
	     	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID100"  target="_blank">投资活动产生现金流量净额</a></td>
	    	<c:forEach var="rsf" items="${rsfList}">
	    		<td class="mid_table_content"><div>${rsf.netCashFlowGeneratedFromInvestmentActivitiesFormated}</div></td>
	    	</c:forEach>
       	</tr>
       	<tr>
	     	<td class="mid_table_content">3</td>
	     	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID101"  target="_blank">筹资活动产生现金流量净额</a></td>
	    	<c:forEach var="rsf" items="${rsfList}">
	    		<td class="mid_table_content"><div>${rsf.netCashFlowGeneratedFromFinancingActivitiesFormated}</div></td>
	    	</c:forEach>
       	</tr>
     		<tr>
		 	<td class="mid_table_content">4</td>
		 	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID102"  target="_blank">公司类型</a></td>
		 	<c:forEach var="rsf" items="${rsfList}">
        <c:set var="scopeOperation" value="'正负负'||'正正负'" />
        <c:set var="companyType" value="${fn:substring(rsf.companyType, 0, fn:length(rsf.companyType))}" />
        <c:choose>
            <c:when test="${scopeOperation.indexOf(companyType) == -1}">
                <td class="mid_table_content"><div style="color: red; font-weight: bold;">${rsf.companyType}</div></td>
            </c:when>
            <c:otherwise>
                <td class="mid_table_content"><div>${rsf.companyType}</div></td>
            </c:otherwise>
        </c:choose>
    </c:forEach>
        </tr>
