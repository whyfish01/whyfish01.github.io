...
<body>
	...
	<div class="mid_title1">毛利率情况（金额单位：元人民币）</div>
	<table style="width:97%">
	   <tbody>
		...
		<tr>
		 	...
		 	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID063"  target="_blank">毛利率=(a-b)÷a</a></td>
		 	<c:forEach var="rsf" items="${rsfList}">
		 		<c:set var="scopeOperation" value="0.4" />
		 		<c:set var="grossMargin" value="${fn:substring(rsf.grossMarginFormated, 0, fn:length(rsf.grossMarginFormated) - 1)}" /> <!-- 提取百分数中的数字部分 -->
		 		<c:choose>
		 			<c:when test="${grossMargin/100 < scopeOperation}">
	    			 	<td class="mid_table_content"><div style="color: red; font-weight: bold;">${rsf.grossMarginFormated}</div></td>
	    			</c:when>
	    			<c:otherwise>
	    			  	<td class="mid_table_content"><div>${rsf.grossMarginFormated}</div></td>
	    			</c:otherwise>
	    		</c:choose>
	    	</c:forEach>
        	</tr>
