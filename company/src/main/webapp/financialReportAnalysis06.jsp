...
<body>
	...
	<div class="mid_title1">固定资产情况（金额单位：元人民币）</div>
	<table style="width:97%">
	   <tbody>
	   	...
	   	<tr>
	     	...
	     	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID033"  target="_blank">a.固定资产</a></td>
	    	<c:forEach var="rsf" items="${rsfList}">
	    		<td class="mid_table_content"><div>${rsf.fixedAssetsFormated}</div></td>
	    	</c:forEach>
       	</tr>
       	<tr>
	     	<td class="mid_table_content">2</td>
	     	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID034"  target="_blank">b.在建工程</a></td>
	    	<c:forEach var="rsf" items="${rsfList}">
	    		<td class="mid_table_content"><div>${rsf.constructionInProgressFormated}</div></td>
	    	</c:forEach>
       	</tr>
       	<tr>
	     	<td class="mid_table_content">3</td>
	     	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID035"  target="_blank">固产合计=a+b</a></td>
	    	<c:forEach var="rsf" items="${rsfList}">
	    		<td class="mid_table_content"><div>${rsf.totalTFxedAssetsFormated}</div></td>
	    	</c:forEach>
       	</tr>
       	<tr>
		 	<td class="mid_table_content">4</td>
		 	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID036"  target="_blank">固产合计占总资产的比率</a></td>
		 	<c:forEach var="rsf" items="${rsfList}">
		 		<c:set var="scopeOperation" value="0.4" />
		 		<c:set var="fixedAssetRatio" value="${fn:substring(rsf.fixedAssetRatioFormated, 0, fn:length(rsf.fixedAssetRatioFormated) - 1)}" /> <!-- 提取百分数中的数字部分 -->
		 		<c:choose>
		 			<c:when test="${fixedAssetRatio/100 > scopeOperation}">
	    			 	<td class="mid_table_content"><div style="color: red; font-weight: bold;">${rsf.fixedAssetRatioFormated}</div></td>
	    			</c:when>
	    			<c:otherwise>
	    			  	<td class="mid_table_content"><div>${rsf.fixedAssetRatioFormated}</div></td>
	    			</c:otherwise>
	    		</c:choose>
	    	</c:forEach>
        	</tr>
