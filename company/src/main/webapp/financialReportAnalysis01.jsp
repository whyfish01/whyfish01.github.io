...
<body>
	...
	<div class="mid_title1">总资产情况（金额单位：元人民币）</div>
	<table style="width:97%">
	   <tbody>
	 	...
	 	<tr>
		 	...
		 	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID003"  target="_blank">总资产增长率</a></td>
		 	<c:forEach var="rsf" items="${rsfList}">
		 		<c:set var="scopeOperation" value="0.1" />
		 		<c:set var="totalAssetsGrowthRate" value="${fn:substring(rsf.totalAssetsGrowthRateFormated, 0, fn:length(rsf.totalAssetsGrowthRateFormated) - 1)}" /> <!-- 提取百分数中的数字部分 -->
		 		<c:choose>
		 			<c:when test="${totalAssetsGrowthRate/100 < scopeOperation}">
	    			 	<td class="mid_table_content"><div style="color: red; font-weight: bold;">${rsf.totalAssetsGrowthRateFormated}</div></td>
	    			</c:when>
	    			<c:otherwise>
	    			  	<td class="mid_table_content"><div>${rsf.totalAssetsGrowthRateFormated}</div></td>
	    			</c:otherwise>
	    		</c:choose>
	    	</c:forEach>
        	</tr>
			<tr>
		 		...
		 		<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID002"  target="_blank">b.净资产(股东权益合计)</a></td>
		 		<c:forEach var="rsf" items="${rsfList}">
	    			 <td class="mid_table_content"><div>${rsf.totalOwnersEquityFormated}</div></td>
	    		</c:forEach>
        	</tr>
			<tr>
		 		<td class="mid_table_content">4</td>
		 		<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID004"  target="_blank">权益乘数=a/b</a></td>
		 		<c:forEach var="rsf" items="${rsfList}">
		 			<c:set var="scopeOperation1" value="2" />
		 			<c:choose>
		 			  	<c:when test="${Double.parseDouble(rsf.equityMultiplierFormated) > scopeOperation1}">
	    			 		<td class="mid_table_content"><div style="color: red; font-weight: bold;">${rsf.equityMultiplierFormated}</div></td>
	    			 	</c:when>
	    			 	<c:otherwise>
	    					 <td class="mid_table_content"><div>${rsf.equityMultiplierFormated}</div></td>
	    			 	</c:otherwise>
	    			</c:choose>
	    		</c:forEach>
        	</tr>
