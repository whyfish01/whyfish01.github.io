...
<body>
	...
	<div class="mid_title1">利润构成情况（金额单位：元人民币）</div>
	<table style="width:97%">
	   <tbody>
	   	...
	   	<tr>
	     	...
	     	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID057"  target="_blank">a.营业收入</a></td>
	    	<c:forEach var="rsf" items="${rsfList}">
       	...
       	<tr>
	     	...
	     	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID062"  target="_blank">b.营业成本</a></td>
	    	<c:forEach var="rsf" items="${rsfList}">
	    		<td class="mid_table_content"><div>${rsf.operatingCostsFormated}</div></td>
	    	</c:forEach>
       	</tr>
       	<tr>
	     	<td class="mid_table_content">3</td>
	     	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID073"  target="_blank">c.税金及附加</a></td>
	    	<c:forEach var="rsf" items="${rsfList}">
	    		<td class="mid_table_content"><div>${rsf.taxesAndSurchargesFormated}</div></td>
	    	</c:forEach>
       	</tr>
       	<tr>
	     	<td class="mid_table_content">4</td>
	     	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID074"  target="_blank">d.四项费用</a></td>
	    	<c:forEach var="rsf" items="${rsfList}">
	    		<td class="mid_table_content"><div>${rsf.fourExpensesFormated}</div></td>
	    	</c:forEach>
       	</tr>
       	<tr>
	     	<td class="mid_table_content">5</td>
	     	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID075"  target="_blank">e.利润总额</a></td>
	    	<c:forEach var="rsf" items="${rsfList}">
       	...
       	<tr>
	     	...
	     	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID076"  target="_blank">f.主营利润=a-b-c-d</a></td>
	    	<c:forEach var="rsf" items="${rsfList}">
	    		<td class="mid_table_content"><div>${rsf.mainProfitFormated}</div></td>
	    	</c:forEach>
       	</tr>
       	<tr>
		 	<td class="mid_table_content">7</td>
		 	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID077"  target="_blank">主营利润率=f÷a</a></td>
		 	<c:forEach var="rsf" items="${rsfList}">
		 		<c:set var="scopeOperation" value="0.2" />
		 		<c:set var="mainProfitMargin" value="${fn:substring(rsf.mainProfitMarginFormated, 0, fn:length(rsf.mainProfitMarginFormated) - 1)}" /> <!-- 提取百分数中的数字部分 -->
		 		<c:choose>
		 			<c:when test="${mainProfitMargin/100 < scopeOperation}">
	    			 	<td class="mid_table_content"><div style="color: red; font-weight: bold;">${rsf.mainProfitMarginFormated}</div></td>
	    			</c:when>
	    			<c:otherwise>
	    			  	<td class="mid_table_content"><div>${rsf.mainProfitMarginFormated}</div></td>
	    			</c:otherwise>
	    		</c:choose>
	    	</c:forEach>
        </tr>
		<tr>
		 	...
		 	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID078"  target="_blank">主营利润占利润总额比例</a></td>
		 	<c:forEach var="rsf" items="${rsfList}">
		 		<c:set var="scopeOperation" value="0.8" />
		 		<c:set var="mainOperatingProfitToTotalProfit" value="${fn:substring(rsf.mainOperatingProfitToTotalProfitFormated, 0, fn:length(rsf.mainOperatingProfitToTotalProfitFormated) - 1)}" /> <!-- 提取百分数中的数字部分 -->
		 		<c:choose>
		 			<c:when test="${mainOperatingProfitToTotalProfit/100 < scopeOperation}">
	    			 	<td class="mid_table_content"><div style="color: red; font-weight: bold;">${rsf.mainOperatingProfitToTotalProfitFormated}</div></td>
	    			</c:when>
	    			<c:otherwise>
	    			  	<td class="mid_table_content"><div>${rsf.mainOperatingProfitToTotalProfitFormated}</div></td>
	    			</c:otherwise>
	    		</c:choose>
	    	</c:forEach>
        </tr>
       <tr>
	     	...
	     	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID079"  target="_blank">g.营业利润</a></td>
	    	<c:forEach var="rsf" items="${rsfList}">
	    		<td class="mid_table_content"><div>${rsf.operatingProfitFormated}</div></td>
	    	</c:forEach>
       	</tr>
       	
       	<tr>
		 	<td class="mid_table_content">10</td>
		 	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID080"  target="_blank">主营利润占营业利润比例</a></td>
		 	<c:forEach var="rsf" items="${rsfList}">
		 		<c:set var="scopeOperation" value="0.3" />
		 		<c:set var="operatingMargin" value="${fn:substring(rsf.operatingMarginFormated, 0, fn:length(rsf.operatingMarginFormated) - 1)}" /> <!-- 提取百分数中的数字部分 -->
		 		<c:choose>
		 			<c:when test="${operatingMargin/100 < scopeOperation}">
	    			 	<td class="mid_table_content"><div style="color: red; font-weight: bold;">${rsf.operatingMarginFormated}</div></td>
	    			</c:when>
	    			<c:otherwise>
	    			  	<td class="mid_table_content"><div>${rsf.operatingMarginFormated}</div></td>
	    			</c:otherwise>
	    		</c:choose>
	    	</c:forEach>
        </tr>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        var myChart6 = new Chart(ctx6, {
            ...
            data: {
                ...
                datasets: [{
                    label: '营业利润',
                    data: chartData6.slice(0, yearList.length),
                    backgroundColor: 'rgba(83, 106, 152, 0.3)', // 设置浅蓝透明背景色
                    borderColor: 'rgba(83, 99, 132, 1)',
                    borderWidth: 1
                }]
