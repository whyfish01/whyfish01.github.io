...
<body>
	...
	<div class="mid_title1">营业收入及增长情况（金额单位：元人民币）</div>
	<table style="width:97%">
	   <tbody>
		...
		<tr>
		 	...
		 	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID058"  target="_blank">营业收入增长率</a></td>
		 	<c:forEach var="rsf" items="${rsfList}">
		 		<c:set var="scopeOperation" value="0.1" />
		 		<c:set var="revenueGrowthRate" value="${fn:substring(rsf.revenueGrowthRateFormated, 0, fn:length(rsf.revenueGrowthRateFormated) - 1)}" /> <!-- 提取百分数中的数字部分 -->
		 		<c:choose>
		 			<c:when test="${revenueGrowthRate/100 < scopeOperation}">
	    			 	<td class="mid_table_content"><div style="color: red; font-weight: bold;">${rsf.revenueGrowthRateFormated}</div></td>
	    			</c:when>
	    			<c:otherwise>
	    			  	<td class="mid_table_content"><div>${rsf.revenueGrowthRateFormated}</div></td>
	    			</c:otherwise>
	    		</c:choose>
	    	</c:forEach>
        	</tr>
       	...
       	<tr>
	     	...
	     	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID001"  target="_blank">a.总资产</a></td>
	    	<c:forEach var="rsf" items="${rsfList}">
       	...
       	<tr>
		 	...
		 	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID059"  target="_blank">总资产周转率=b/a*100%</a></td>
		 	<c:forEach var="rsf" items="${rsfList}">
		 		<c:set var="scopeOperation" value="0.35" />
		 		<c:set var="totalAssetTurnover" value="${fn:substring(rsf.totalAssetTurnoverFormated, 0, fn:length(rsf.totalAssetTurnoverFormated) - 1)}" /> <!-- 提取百分数中的数字部分 -->
		 		<c:choose>
		 			<c:when test="${totalAssetTurnover/100 < scopeOperation}">
	    			 	<td class="mid_table_content"><div style="color: red; font-weight: bold;">${rsf.totalAssetTurnoverFormated}</div></td>
	    			</c:when>
	    			<c:otherwise>
	    			  	<td class="mid_table_content"><div>${rsf.totalAssetTurnoverFormated}</div></td>
	    			</c:otherwise>
	    		</c:choose>
	    	</c:forEach>
        	</tr>
 		...
 		<tr>
	     	...
	     	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID060"  target="_blank">c.销售商品、提供劳务收到的现金</a></td>
	    	<c:forEach var="rsf" items="${rsfList}">
	    		<td class="mid_table_content"><div>${rsf.cashReceivedFromSellingGoodsAndProvidingServicesFormated}</div></td>
	    	</c:forEach>
       	</tr>
       	<tr>
		 	<td class="mid_table_content">6</td>
		 	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID061"  target="_blank">销售商品、提供劳务收到的现金营收占比</a></td>
		 	<c:forEach var="rsf" items="${rsfList}">
		 		<c:set var="scopeOperation" value="0.35" />
		 		<c:set var="proportionOfCashSellingGoodsProvidingServices" value="${fn:substring(rsf.proportionOfCashSellingGoodsProvidingServicesFormated, 0, fn:length(rsf.proportionOfCashSellingGoodsProvidingServicesFormated) - 1)}" /> <!-- 提取百分数中的数字部分 -->
		 		<c:choose>
		 			<c:when test="${proportionOfCashSellingGoodsProvidingServices/100 < scopeOperation}">
	    			 	<td class="mid_table_content"><div style="color: red; font-weight: bold;">${rsf.proportionOfCashSellingGoodsProvidingServicesFormated}</div></td>
	    			</c:when>
	    			<c:otherwise>
	    			  	<td class="mid_table_content"><div>${rsf.proportionOfCashSellingGoodsProvidingServicesFormated}</div></td>
	    			</c:otherwise>
	    		</c:choose>
	    	</c:forEach>
        	</tr>
