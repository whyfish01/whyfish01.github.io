...
<body>
	...
	<div class="mid_title1">投资类资产情况（金额单位：元人民币）</div>
	<table style="width:97%">
	   <tbody>
	   	...
	   	<tr>
	     	...
	     	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID001"  target="_blank">总资产</a></td>
	    	<c:forEach var="rsf" items="${rsfList}">
       	...
       	<tr>
	     	...
	     	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID037"  target="_blank">a.交易性金融资产</a></td>
	    	<c:forEach var="rsf" items="${rsfList}">
	    		<td class="mid_table_content"><div>${rsf.tradingFinancialAssetsFormated}</div></td>
	    	</c:forEach>
       	</tr>
       	<tr>
	     	<td class="mid_table_content">3</td>
	     	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID038"  target="_blank">b.其他非流动金融资产</a></td>
	    	<c:forEach var="rsf" items="${rsfList}">
	    		<td class="mid_table_content"><div>${rsf.otherNonCurrentFinancialAssetsFormated}</div></td>
	    	</c:forEach>
       	</tr>
       	<tr>
	     	<td class="mid_table_content">4</td>
	     	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID039"  target="_blank">c.投资性房地产</a></td>
	    	<c:forEach var="rsf" items="${rsfList}">
	    		<td class="mid_table_content"><div>${rsf.investmentRealEstateFormated}</div></td>
	    	</c:forEach>
       	</tr>
       	<tr>
	     	<td class="mid_table_content">5</td>
	     	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID040"  target="_blank">d.长期股权投资</a></td>
	    	<c:forEach var="rsf" items="${rsfList}">
	    		<td class="mid_table_content"><div>${rsf.longTermEquityInvestmentFormated}</div></td>
	    	</c:forEach>
       	</tr>
       	<tr>
		 	<td class="mid_table_content">6</td>
		 	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID041"  target="_blank">投资类资产占总资产的比率</a></td>
		 	<c:forEach var="rsf" items="${rsfList}">
		 		<c:set var="scopeOperation" value="0.1" />
		 		<c:set var="investmentAssetRatio" value="${fn:substring(rsf.investmentAssetRatioFormated, 0, fn:length(rsf.investmentAssetRatioFormated) - 1)}" /> <!-- 提取百分数中的数字部分 -->
		 		<c:choose>
		 			<c:when test="${investmentAssetRatio/100 > scopeOperation}">
	    			 	<td class="mid_table_content"><div style="color: red; font-weight: bold;">${rsf.investmentAssetRatioFormated}</div></td>
	    			</c:when>
	    			<c:otherwise>
	    			  	<td class="mid_table_content"><div>${rsf.investmentAssetRatioFormated}</div></td>
	    			</c:otherwise>
	    		</c:choose>
	    	</c:forEach>
        	</tr>
