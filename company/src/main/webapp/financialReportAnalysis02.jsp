...
<body>
	...
	<div class="mid_title1">资产负债率情况（金额单位：元人民币）</div>
	   ...
	   <tbody>
	   	...
	   	<tr>
	    	...
	    	<c:forEach var="rsf" items="${rsfList}">
	     	...
	     	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID005"  target="_blank">总负债</a></td>
	    	<c:forEach var="rsf" items="${rsfList}">
	    		<td class="mid_table_content"><div>${rsf.totalLiabilitiesFormated}</div></td>
	    	</c:forEach>
       	</tr>
	 	<tr>
		 	<td class="mid_table_content">3</td>
		 	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID006"  target="_blank">资产负债率</a></td>
		 	<c:forEach var="rsf" items="${rsfList}">
		 		<c:set var="scopeOperation" value="0.4" />
		 		<c:set var="assetLiabilityRatio" value="${fn:substring(rsf.assetLiabilityRatioFormated, 0, fn:length(rsf.assetLiabilityRatioFormated) - 1)}" /> <!-- 提取百分数中的数字部分 -->
		 		<c:choose>
		 			<c:when test="${assetLiabilityRatio/100 > scopeOperation}">
	    			 	<td class="mid_table_content"><div style="color: red; font-weight: bold;">${rsf.assetLiabilityRatioFormated}</div></td>
	    			</c:when>
	    			<c:otherwise>
	    			  	<td class="mid_table_content"><div>${rsf.assetLiabilityRatioFormated}</div></td>
	    			</c:otherwise>
	    		</c:choose>
	    	</c:forEach>
        	</tr>
