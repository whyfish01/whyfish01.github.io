...
<body>
	...
	<div class="mid_title1">净利润情况（金额单位：元人民币）</div>
	<table style="width:97%">
	   <tbody>
	   	...
	   	<tr>
	     	...
	     	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID081"  target="_blank">a.净利润</a></td>
	    	<c:forEach var="rsf" items="${rsfList}">
	    		<td class="mid_table_content"><div>${rsf.netProfitFormated}</div></td>
	    	</c:forEach>
       	</tr>
       	<tr>
		 	<td class="mid_table_content">2</td>
		 	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID082"  target="_blank">净利润增长率</a></td>
		 	<c:forEach var="rsf" items="${rsfList}">
		 		<c:set var="scopeOperation" value="0.1" />
		 		<c:set var="netProfitGrowthRate" value="${fn:substring(rsf.netProfitGrowthRateFormated, 0, fn:length(rsf.netProfitGrowthRateFormated) - 1)}" /> <!-- 提取百分数中的数字部分 -->
		 		<c:choose>
		 			<c:when test="${netProfitGrowthRate/100 < scopeOperation}">
	    			 	<td class="mid_table_content"><div style="color: red; font-weight: bold;">${rsf.netProfitGrowthRateFormated}</div></td>
	    			</c:when>
	    			<c:otherwise>
	    			  	<td class="mid_table_content"><div>${rsf.netProfitGrowthRateFormated}</div></td>
	    			</c:otherwise>
	    		</c:choose>
	    	</c:forEach>
        </tr>
       	<tr>
	     	...
	     	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID057"  target="_blank">b.营业收入</a></td>
	    	<c:forEach var="rsf" items="${rsfList}">
		 	...
		 	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID083"  target="_blank">净利润率=a/b*100%</a></td>
		 	<c:forEach var="rsf" items="${rsfList}">
		 		<c:set var="scopeOperation" value="0.1" />
		 		<c:set var="netProfitMargin" value="${fn:substring(rsf.netProfitMarginFormated, 0, fn:length(rsf.netProfitMarginFormated) - 1)}" /> <!-- 提取百分数中的数字部分 -->
		 		<c:choose>
		 			<c:when test="${netProfitMargin/100 < scopeOperation}">
	    			 	<td class="mid_table_content"><div style="color: red; font-weight: bold;">${rsf.netProfitMarginFormated}</div></td>
	    			</c:when>
	    			<c:otherwise>
	    			  	<td class="mid_table_content"><div>${rsf.netProfitMarginFormated}</div></td>
	    			</c:otherwise>
	    		</c:choose>
	    	</c:forEach>
        </tr>
       	<tr>
	     	...
	     	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID084"  target="_blank">b.经营活动产生的现金流量净额</a></td>
	    	<c:forEach var="rsf" items="${rsfList}">
       	...
       	<tr>
		 	...
		 	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID085"  target="_blank">经营活动产生的现金流量净额增长率</a></td>
		 	<c:forEach var="rsf" items="${rsfList}">
		 		<c:set var="scopeOperation" value="0" />
		 		<c:set var="growthRateOfNetCashFlowFromOperating" value="${fn:substring(rsf.growthRateOfNetCashFlowFromOperatingFormated, 0, fn:length(rsf.growthRateOfNetCashFlowFromOperatingFormated) - 1)}" /> <!-- 提取百分数中的数字部分 -->
		 		<c:choose>
		 			<c:when test="${growthRateOfNetCashFlowFromOperating/100 < scopeOperation}">
	    			 	<td class="mid_table_content"><div style="color: red; font-weight: bold;">${rsf.growthRateOfNetCashFlowFromOperatingFormated}</div></td>
	    			</c:when>
	    			<c:otherwise>
	    			  	<td class="mid_table_content"><div>${rsf.growthRateOfNetCashFlowFromOperatingFormated}</div></td>
	    			</c:otherwise>
	    		</c:choose>
	    	</c:forEach>
        </tr>
       	<tr>
		 	...
		 	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID086"  target="_blank">净利润现金比率=b÷a×100%</a></td>
		 	<c:forEach var="rsf" items="${rsfList}">
		 		<c:set var="scopeOperation" value="1" />
		 		<c:set var="netProfitCashRatio" value="${fn:substring(rsf.netProfitCashRatioFormated, 0, fn:length(rsf.netProfitCashRatioFormated) - 1)}" /> <!-- 提取百分数中的数字部分 -->
		 		<c:choose>
		 			<c:when test="${netProfitCashRatio/100 < scopeOperation}">
	    			 	<td class="mid_table_content"><div style="color: red; font-weight: bold;">${rsf.netProfitCashRatioFormated}</div></td>
	    			</c:when>
	    			<c:otherwise>
	    			  	<td class="mid_table_content"><div>${rsf.netProfitCashRatioFormated}</div></td>
	    			</c:otherwise>
	    		</c:choose>
	    	</c:forEach>
        </tr>
