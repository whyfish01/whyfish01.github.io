...
<body>
	...
	<div class="mid_title1">准货币资金（金额单位：元人民币）</div>
	   ...
	   <tbody>
	   	...
	   	<tr>
	    	...
	    	<c:forEach var="rsf" items="${rsfList}">
	     	...
	     	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID015"  target="_blank">b.交易性金融资产中的理财产品</a></td>
	    	<c:forEach var="rsf" items="${rsfList}">
	    		<td class="mid_table_content"><div>${rsf.tradingFinancialAssetsManagementFormated}</div></td>
	    	</c:forEach>
       	</tr>
	 	<tr>
		 	<td class="mid_table_content">3</td>
		 	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID016"  target="_blank">c.其他流动资产的理财产品</a></td>
		 	<c:forEach var="rsf" items="${rsfList}">
	    		<td class="mid_table_content"><div>${rsf.otherCurrentAssetsManagementFormated}</div></td>
	    	</c:forEach>
        </tr>
        <tr>
		 	<td class="mid_table_content">4</td>
		 	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID017"  target="_blank">d.准货币资金=a+b+c</a></td>
		 	<c:forEach var="rsf" items="${rsfList}">
	    		<td class="mid_table_content"><div>${rsf.quasiMonetaryFormated}</div></td>
	    	</c:forEach>
        </tr>
        <tr>
		 	<td class="mid_table_content">5</td>
		 	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID008"  target="_blank">e.短期借款</a></td>
		 	<c:forEach var="rsf" items="${rsfList}">
        ...
        <tr>
		 	...
		 	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID009"  target="_blank">f.一年内到期的非流动负债</a></td>
		 	<c:forEach var="rsf" items="${rsfList}">
        ...
        <tr>
		 	...
		 	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID018"  target="_blank">g.短期有息负债=e+f</a></td>
		 	<c:forEach var="rsf" items="${rsfList}">
	    		<td class="mid_table_content"><div>${rsf.shortTermForeignDebtFormated}</div></td>
	    	</c:forEach>
        </tr>
        <tr>
		 	<td class="mid_table_content">8</td>
		 	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID019"  target="_blank">h.准货币资金-短期有息负债=d-g</a></td>
		 	<c:forEach var="rsf" items="${rsfList}">
		 		<c:set var="scopeOperation" value="0" />
		 		<c:set var="shortTermDebtRepaymentRisk" value="${rsf.shortTermDebtRepaymentRiskFormated}" />
		 		<c:choose>
		 			<c:when test="${shortTermDebtRepaymentRisk < scopeOperation}">
	    		<td class="mid_table_content"><div style="color: red; font-weight: bold;">${rsf.shortTermDebtRepaymentRiskFormated}</div></td>
	    			</c:when>
	    			<c:otherwise>
	    			  	<td class="mid_table_content"><div>${rsf.shortTermDebtRepaymentRiskFormated}</div></td>
	    			</c:otherwise>
	    	</c:choose>
	    	</c:forEach>
        </tr>
        <tr>
		 	...
		 	<td class="mid_table_content"><a href="IndicatorDetailForLook?indicatorID=ID020"  target="_blank">准货币资金占总资产比率</a></td>
		 	<c:forEach var="rsf" items="${rsfList}">
		 		<c:set var="scopeOperation" value="0.25" />
		 		<c:set var="quasiMonetaryCapitalRatio" value="${fn:substring(rsf.quasiMonetaryCapitalRatioFormated, 0, fn:length(rsf.quasiMonetaryCapitalRatioFormated) - 1)}" /><!-- 提取百分数中的数字部分 -->
		 		<c:choose>
		 			<c:when test="${quasiMonetaryCapitalRatio/100 < scopeOperation}">
	    			 	<td class="mid_table_content"><div style="color: red; font-weight: bold;">${rsf.quasiMonetaryCapitalRatioFormated}</div></td>
	    			</c:when>
	    			<c:otherwise>
	    			  	<td class="mid_table_content"><div>${rsf.quasiMonetaryCapitalRatioFormated}</div></td>
	    			</c:otherwise>
	    		</c:choose>
	    	</c:forEach>
        	</tr>
