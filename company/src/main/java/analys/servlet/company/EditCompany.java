...
public class EditCompany extends HttpServlet {
    ...
    public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(companyNameIsNull) {
            // 如果为空
            request.setAttribute("a", "修改失败：企业全称不能为空");
        } else if(AShareNameIsNull){
        	// 如果为空
            request.setAttribute("a", "修改失败：A股简称不能为空");
        } else if(AShareCodeIsNull) {
        	// 如果为空
        	request.setAttribute("a", "修改失败：A股代码不能为空");
    	} else if(industry1IsNull) {
    		// 如果为空
        	request.setAttribute("a", "修改失败：一级行业不能为空");
    	} else if(industry2IsNull) {
    		// 如果为空
    		request.setAttribute("a", "修改失败：二级行业不能为空");
    	} else if(industry3IsNull) {
        	// 如果为空
    		request.setAttribute("a", "修改失败：三级行业不能为空");
    	} else {
    		...
    		// 插入数据库
    		boolean EditResult = cdi.updateCompany(cop);
        	if(EditResult) {
                // 添加成功，设置成功消息
