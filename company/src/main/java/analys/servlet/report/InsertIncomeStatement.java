...
public class InsertIncomeStatement extends HttpServlet {
    ...
    public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	...
    	try {
    	    ...
    	    // 处理转换异常，例如设置默认值或记录日志
    	    System.out.println("输入的数据无法转换为有效的数字格式，默认设置为0.00");
    	    reclassificationOfFinancialAssetsInOtherComprehensiveIncome = 0.00;
    	}
    	...
    	is.setCreateDateTime(createTime);
    	is.setUpdateDateTime(updateTime);
    	
    	System.out.print(is.getReclassificationOfFinancialAssetsInOtherComprehensiveIncome());
    	
    	boolean insertResult=rdi.insertIncomeStatement(is);
    	if(insertResult) {
            // 添加成功，设置成功消息
    	...
    	RequestDispatcher dispatcher=request.getRequestDispatcher("financialReportAdd1.jsp");
