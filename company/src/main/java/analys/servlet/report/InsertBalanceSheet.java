...
public class InsertBalanceSheet extends HttpServlet {
    ...
    public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	...
    	//设置财报编号
    	String financialReportingNo=null;
        financialReportingNo=AShareCode+financialReportingYear; 
        //System.out.println(financialReportingNo);
        ...
        @SuppressWarnings("static-access")
		boolean auditOpinionisNull=dl.isNullOrEmpty(auditOpinion);
        if(AShareCodeisNull) {
            // 如果为空
            request.setAttribute("a", "添加失败：A股代码不能为空");
        } else if(financialReportingYearisNull) {
        	// 如果为空
            request.setAttribute("a", "添加失败：财报年份不能为空");
        }else if(auditOpinionisNull) {
        	// 如果为空
            request.setAttribute("a", "添加失败：审计意见不能为空");
        }
        else {
        	bs.setFinancialReportingNo(financialReportingNo);
            bs.setAShareCode(AShareCode);
            bs.setAShareName(AShareName);
            bs.setFinancialReportingYear(financialReportingYear);
            bs.setAuditOpinion(auditOpinion);
        	...
        	// 插入数据库
        	boolean insertResult = rdi.insertBalanceSheet(bs);
        	if(insertResult) {
                // 添加成功，设置成功消息
        ...
        RequestDispatcher dispatcher=request.getRequestDispatcher("financialReportAdd.jsp");
