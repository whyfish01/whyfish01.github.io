...
public class GetInvestmentAssetRatioForYears extends HttpServlet {
private static final long serialVersionUID = 1L;
    ...
    public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	...
    	List<ReportAnalysis> anList= aadi.getReportAnalysisForCompany07(AShareCode);
    	...
    	// 选择第一组需要的指标并存储在新的List中：投资类资产占总资产的比率
    	List<Double> chartData1 = new ArrayList<>();
    	for (Reportsummary ry : ryList) {
    		Double investmentAssetRatio = ry.getInvestmentAssetRatio();
    	    chartData1.add(investmentAssetRatio);
    	    
    	}
    	request.setAttribute("chartData1", chartData1);
    	
    	// 选择第二组需要的指标并存储在新的List中：交易性金融资产
    	List<Double> chartData2 = new ArrayList<>();
    	for (Reportsummary ry : ryList) {
    		Double tradingFinancialAssets = ry.getTradingFinancialAssets();
    	    chartData2.add(tradingFinancialAssets);
    	    
    	}
    	request.setAttribute("chartData2", chartData2);
    	
    	
    	// 选择第三组需要的指标并存储在新的List中：其他非流动金融资产
    	List<Double> chartData3 = new ArrayList<>();
    	for (Reportsummary ry : ryList) {
    		Double otherNonCurrentFinancialAssets = ry.getOtherNonCurrentFinancialAssets();
    	    chartData3.add(otherNonCurrentFinancialAssets);
    	    
    	}
    	request.setAttribute("chartData3", chartData3);
    	
    	// 选择第四组需要的指标并存储在新的List中：长期股权投资
    	List<Double> chartData4 = new ArrayList<>();
    	for (Reportsummary ry : ryList) {
    		Double longTermEquityInvestment = ry.getLongTermEquityInvestment();
    	    chartData4.add(longTermEquityInvestment);
    	    
    	}
    	request.setAttribute("chartData4", chartData4);
    	
    	RequestDispatcher dispatcher=request.getRequestDispatcher("financialReportAnalysis07.jsp");
