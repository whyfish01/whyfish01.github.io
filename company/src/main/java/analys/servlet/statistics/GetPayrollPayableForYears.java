...
public class GetPayrollPayableForYears extends HttpServlet {
private static final long serialVersionUID = 1L;
    ...
    public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	...
    	List<ReportAnalysis> anList= aadi.getReportAnalysisForCompany09(AShareCode);
    	...
    	// 选择第一组需要的指标并存储在新的List中：当年职工总薪酬
    	List<Double> chartData1 = new ArrayList<>();
    	for (Reportsummary ry : ryList) {
    		Double totalEmployeeCompensation = ry.getTotalEmployeeCompensation();
    	    chartData1.add(totalEmployeeCompensation);
    	    
    	}
    	request.setAttribute("chartData1", chartData1);
    	
    	// 选择第二组需要的指标并存储在新的List中：人均月薪
    	List<Double> chartData2 = new ArrayList<>();
    	for (Reportsummary ry : ryList) {
    		Double perCapitaMonthlySalary = ry.getPerCapitaMonthlySalary();
    	    chartData2.add(perCapitaMonthlySalary);
    	    
    	}
    	request.setAttribute("chartData2", chartData2);
    	
    	
    	RequestDispatcher dispatcher=request.getRequestDispatcher("financialReportAnalysis09.jsp");
