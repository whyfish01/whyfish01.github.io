...
public class GetTotalTFxedAssetsForYears extends HttpServlet {
private static final long serialVersionUID = 1L;
    ...
    public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	...
    	List<ReportAnalysis> anList= aadi.getReportAnalysisForCompany06(AShareCode);
    	...
    	// 选择第一组需要的指标并存储在新的List中：固产合计
    	List<Double> chartData1 = new ArrayList<>();
    	for (Reportsummary ry : ryList) {
    		Double totalTFxedAssets = ry.getTotalTFxedAssets();
    	    chartData1.add(totalTFxedAssets);
    	    
    	}
    	request.setAttribute("chartData1", chartData1);
    	
    	// 选择第二组需要的指标并存储在新的List中：固产合计占总资产的比率
    	List<Double> chartData2 = new ArrayList<>();
    	for (Reportsummary ry : ryList) {
    		Double fixedAssetRatio = ry.getFixedAssetRatio();
    	    chartData2.add(fixedAssetRatio);
    	}
    	request.setAttribute("chartData2", chartData2);
    	
    	
    	RequestDispatcher dispatcher=request.getRequestDispatcher("financialReportAnalysis06.jsp");
