...
public class GetCompanyTypeForYears extends HttpServlet {
    ...
    public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	...
    	List<ReportAnalysis> anList= aadi.getReportAnalysisForCompany19(AShareCode);
    	...
    	// 选择第二组需要的指标并存储在新的List中：投资活动产生的现金流量净额
    	List<Double> chartData2 = new ArrayList<>();
    	for (Reportsummary ry : ryList) {
    		Double netCashFlowGeneratedFromInvestmentActivities = ry.getNetCashFlowGeneratedFromInvestmentActivities();
    	    chartData2.add(netCashFlowGeneratedFromInvestmentActivities);
   
    	}
    	request.setAttribute("chartData2", chartData2);
    	
    	// 选择第三组需要的指标并存储在新的List中：筹资活动产生的现金流量净额
    	List<Double> chartData3 = new ArrayList<>();
    	for (Reportsummary ry : ryList) {
    		Double netCashFlowGeneratedFromFinancingActivities = ry.getNetCashFlowGeneratedFromFinancingActivities();
    	    chartData3.add(netCashFlowGeneratedFromFinancingActivities);
   
    	}
    	request.setAttribute("chartData3", chartData3);
    	
    	RequestDispatcher dispatcher=request.getRequestDispatcher("financialReportAnalysis19.jsp");
