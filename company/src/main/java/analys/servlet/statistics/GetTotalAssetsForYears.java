...
public class GetTotalAssetsForYears extends HttpServlet {
private static final long serialVersionUID = 1L;
    ...
    public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	...
    	List<ReportAnalysis> anList= aadi.getReportAnalysisForCompany01(AShareCode);
    	...
    	for (Reportsummary ry : ryList) {
    	    Double totalOwnersEquity = ry.getTotalOwnersEquity();
    	    chartData2.add(totalOwnersEquity);
    	}
    	...
    	for (Reportsummary ry : ryList) {
    	    Double totalAssetsGrowthRate = ry.getTotalAssetsGrowthRate();
    	    chartData3.add(totalAssetsGrowthRate);
    	    
    	}
    	request.setAttribute("chartData3", chartData3);

    	// 选择第四组需要的指标并存储在新的List中 	
    	List<Double> chartData4 = new ArrayList<>();
    	for (Reportsummary ry : ryList) {
    	    Double equityMultiplier = ry.getEquityMultiplier();
    	    chartData4.add(equityMultiplier);
    	}
    	request.setAttribute("chartData4", chartData4);
    	
    	RequestDispatcher dispatcher=request.getRequestDispatcher("financialReportAnalysis01.jsp");
